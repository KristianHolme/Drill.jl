...
@testitem "Buffer logprobs consistency" tags = [:buffers, :rollouts] setup = [SharedTestSetup] begin
    ...
    policy = ActorCriticLayer(Drill.observation_space(env), Drill.action_space(env))
    ...
    for i in 1:10
        ...
        _, new_logprobs, _, _ = Drill.evaluate_actions(policy, obs, act, ps, st)
...
@testitem "RolloutBuffer with discrete actions" tags = [:buffers, :rollouts, :discrete] setup = [SharedTestSetup] begin
    ...
    policy = DiscreteActorCriticLayer(Drill.observation_space(env), Drill.action_space(env))
    alg = PPO(; n_steps = 8, batch_size = 8, epochs = 1)
    agent = Agent(policy, alg; verbose = 0)
    ...
    @test all(map(a -> a ∈ Drill.action_space(env), actions))
    ...
    ps = agent.train_state.parameters
    st = agent.train_state.states
    eval_values, eval_logprobs, entropy, _ = Drill.evaluate_actions(policy, obs, actions, ps, st)
...
@testitem "Discrete vs continuous buffer comparison" tags = [:buffers, :rollouts, :comparison] setup = [SharedTestSetup] begin
    ...
    discrete_policy = DiscreteActorCriticLayer(Drill.observation_space(discrete_env), Drill.action_space(discrete_env))
    ...
    continuous_policy = ContinuousActorCriticLayer(Drill.observation_space(continuous_env), Drill.action_space(continuous_env))
    ...
    @test all(map(a -> a ∈ Drill.action_space(discrete_env), discrete_actions))
...
@testitem "Basic ReplayBuffer workings" tags = [:buffers, :rollouts] setup = [SharedTestSetup] begin
    ...
    policy = ContinuousActorCriticLayer(Drill.observation_space(env), Drill.action_space(env), critic_type = QCritic())
