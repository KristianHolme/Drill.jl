...
@testset "PPO smoke test improves tracking env performance" begin
    ...
    layer = ActorCriticModel(obs_space, act_space; hidden_dims = [64, 64])
    ...
    baseline_stats = evaluate(extract_policy(cache), baseline_env; n_eval_episodes = 64, deterministic = true, warn = false)
    baseline_mean_step = baseline_stats.mean_reward / baseline_stats.mean_length
    @test baseline_mean_step < 0.6f0

    solve!(cache)

    trained_stats = evaluate(extract_policy(cache), trained_eval_env; n_eval_episodes = 64, deterministic = true, warn = false)
...
@testset "PPO agent serialization roundtrip" begin
    ...
    layer = ActorCriticModel(obs_space, act_space; hidden_dims = [32, 32])
    ...
    mktempdir() do dir
        ...
        saved_path = save_model_params_and_state(cache, path)

        new_layer = ActorCriticModel(obs_space, act_space; hidden_dims = [32, 32])
        new_cache = init(
        ...
        load_model_params_and_state!(new_cache, alg, saved_path)
