...
# CI hang diagnostics (temporary): flushed markers + hard timeout so ParallelTestRunner
# cannot sit until the 60m job limit with no signal. Remove once the hang is fixed.
const TEST_LOGGING_TIMEOUT_SEC = 15 * 60
...
# Wall-clock timeout around Wandb + TB + DearDiary. On expiry, abort the worker so
# ParallelTestRunner does not block forever on a wedged process / wandb-core.
_test_logging_backends_timed_out = Ref(false)
_test_logging_backends_timer = Timer(float(TEST_LOGGING_TIMEOUT_SEC)) do _
    _test_logging_backends_timed_out[] = true
    _test_logging_diag(
        "TIMEOUT after $(TEST_LOGGING_TIMEOUT_SEC)s during Wandb+TB+DearDiary — aborting worker",
    )
    exit(124)
end
...
try
    ...
    close(_test_logging_backends_timer)
    if _test_logging_backends_timed_out[]
        error(
            "test_logging timed out after $(TEST_LOGGING_TIMEOUT_SEC)s during Wandb+TB+DearDiary",
        )
    end
end
