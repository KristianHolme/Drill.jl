...
"""
    evaluate(policy::AbstractPolicy, env; kwargs...)
...
# Arguments
- `policy`: [`AbstractPolicy`](@ref) whose actions are evaluated (e.g. [`NeuralPolicy`](@ref) from [`extract_policy`](@ref))
...
- `rng::AbstractRNG = default_rng()`: RNG passed to the policy
...
stats = evaluate(policy, env; n_eval_episodes = 20)
stats.mean_reward, stats.std_reward

# Or destructure the four summary values
mean_reward, std_reward, mean_length, std_length =
    evaluate(policy, env; n_eval_episodes = 20)
...
episode_rewards, episode_lengths = evaluate(
    policy, env; n_eval_episodes = 20, return_stats = false
)

# Fail if mean episode return is too low
evaluate(policy, env; reward_threshold = 100.0, n_eval_episodes = 50)
```
"""
function evaluate(
