...
_runtime_model(model_or_cache) = hasproperty(model_or_cache, :model) ? model_or_cache.model : model_or_cache
...
function execute_rollout_action_values(
    ...
    return rollout_action_values_kernel(_runtime_model(model), obs_batch, ps, st, rng)
end
...
function execute_rollout_predict_actions(
    ...
    if deterministic
        return rollout_predict_actions_deterministic_kernel(_runtime_model(model), obs_batch, ps, st)
    end
    return rollout_predict_actions_stochastic_kernel(_runtime_model(model), obs_batch, ps, st, rng)
end
...
function execute_rollout_predict_values(
    ...
    return rollout_predict_values_kernel(_runtime_model(model), obs_batch, ps, st)
end
...
function execute_deployment_predict_actions(
    ...
    if deterministic
        return deployment_predict_actions_deterministic_kernel(_runtime_model(model), obs_batch, ps, st)
    end
    return deployment_predict_actions_stochastic_kernel(_runtime_model(model), obs_batch, ps, st, rng)
end
